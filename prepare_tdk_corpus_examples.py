...
from prepare_tdk_idiom_examples import (  # noqa: E402
    extract_examples, find_span, idiom_stems, tokenize, tr_lower,
)
...
def build_stem_map(analyzer) -> dict[str, str]:
    ...
    for i, tok in enumerate(vocab):
        ...
        sm[tok] = analyzer.analyze(tok).stem or tok
    STEM_MAP.write_text(json.dumps(sm, ensure_ascii=False), encoding="utf-8")
...
def main() -> None:
    ...
    stem_map = build_stem_map(analyzer)
    ...
    for r in rows:
        seq = idiom_stems(r["text"], analyzer)
        if len(seq) < 2:
            continue
